#### PRACTICA 5 ####
# Vazquez Blanco, Oscar
# Alonso Astruga, Juan
.data

introduce: .asciiz "Introduzca numero decimal: "
error_longitud_fin: .asciiz "El numero introducido es demasiado largo. Max. 10 digitos."
error_caracter_fin: .asciiz "Ha introducido algun caracter no permitido. Permitidos [(-)0-9]"
decimal: .space 80	# Valor arbitrario, cuando se quiere introducir numero se da a Enter
...
la $a0, decimal
li $a1, 80	# Valor arbitrario, cuando se quiere introducir numero se da a Enter
li $v0, 8
syscall

jal funcion
li $t0, 1	# Para comprobar error de longitud
li $t1, 2	# Para comprobar error de caracter incorrecto

beq $v0, $t0, fin_error_longitud
beq $v0, $t1, fin_error_caracter_incorrecto
...
fin_error_longitud:
	la $a0, error_longitud_fin
	li $v0, 4
	syscall		# Imprime correspondiente mensaje de error
	
	li $v0, 10
	syscall		# Fin del programa
	
fin_error_caracter_incorrecto:
	la $a0, error_caracter_fin
	li $v0, 4
	syscall		# Imprime correspondiente mensaje de error
